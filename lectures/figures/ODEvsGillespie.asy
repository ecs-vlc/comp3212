...
Solution sol = integrate(X0, func, 0, T, h=0.05, RK4);
...
show = true;
for(real x=0; x<=xmax; x+= 1) {
  if (show) {
    draw(s+(x,0)--s+(x,0.02));
    draw(s+(x,2)--s+(x,2-0.02));
    label(string(x,2), s+(x,0), S);
  } else {
    draw(s+(x,0)--s+(x,0.01));
...
for(int t=0; t<sol.t.length; ++t) {
  write(t, sol.t[t]);
  write(X[1], X[2], sol.t[t], sol.y[t][0], sol.y[t][1]);
